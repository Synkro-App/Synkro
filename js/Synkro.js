...
let listaSeccionesFirebase = [];

// Campos locales sincronizados
let edadUsuarioGlobal = 0; 
let subsUsuarioGlobal = ""; 

// Variable para recordar qué pantalla abrir tras confirmar el modal de mixtas
let pantallaMixtaPendiente = "";

// --- CÁLCULO RIGUROSO DE EDAD (DD-MM-AAAA) ---
function calcularEdadCompleta(fechaNacimientoStr) {
    if (!fechaNacimientoStr) return 0;
    
    // Convertir de forma segura asumiendo que el formato nativo/registro viene como YYYY-MM-DD
    // Si viniera como DD-MM-YYYY, se rompería al pasar al constructor, por lo que estandarizamos el split:
    let ano, mes, dia;
    ...
    // Si el mes actual es menor al de nacimiento, o es el mismo mes pero hoy es un día anterior -> No ha cumplido
...
// Renderizar tarjetas evaluando los criterios del filtro actual
function renderizarSecciones(idioma) {
    const contenedor = document.getElementById("sections-container");
    if (!contenedor) return;

    contenedor.innerHTML = ""; 

    listaSeccionesFirebase.forEach(docSeccion => {
        const datos = docSeccion.data();
        
        const tipoSeccion = datos.type || "Gratis";
        const ageSeccion = datos.age !== undefined ? Number(datos.age) : 0; 
        const arraySubsSeccion = Array.isArray(datos.subs) ? datos.subs : [];
        const pantallaDestino = datos.pantalla || "";

        // --- FILTRO DE EDAD LITERAL: age <= edadUsuarioGlobal ---
        if (ageSeccion > edadUsuarioGlobal) {
            return; 
        }

        // --- FILTRO PARA SECCIONES DE PAGO ---
        if (tipoSeccion === "Pago") {
            const pasaFiltroSubs = arraySubsSeccion.includes(subsUsuarioGlobal);
            if (!pasaFiltroSubs) {
                return; 
            }
        }

        let nombreSeccion = "Sin nombre";
        if (datos.nombre) {
            if (typeof datos.nombre === "object") {
                nombreSeccion = datos.nombre[idioma] || datos.nombre["es"] || "Sección";
            } else {
                nombreSeccion = datos.nombre;
            }
        }

        const rutaImagen = datos.imagen || "default-section.png";

        const tarjeta = document.createElement("div");
        tarjeta.className = "section-card";
        tarjeta.dataset.id = docSeccion.id;

        tarjeta.innerHTML = `
            <img src="${rutaImagen}" alt="${nombreSeccion}" class="section-img" onerror="this.src='default-section.png'">
            <span class="section-name">${nombreSeccion}</span>
        `;
        ...
        tarjeta.addEventListener("click", () => {
            ...
            if (tipoSeccion === "Gratis" || tipoSeccion === "Pago") {
                window.location.href = `${pantallaDestino}.html`;
            } else if (tipoSeccion === "Mixta") {
                // Abrimos el Pop-up guardando la referencia de la pantalla destino
                pantallaMixtaPendiente = pantallaDestino;
                const modal = document.getElementById("popup-mixta");
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
        ...
        console.error("Error cargando idiomas.json en el Hub Real:", error);
    }
    ...
    if (selectorIdioma) {
        ...
        selectorIdioma.addEventListener("change", (e) => {
            ...
            renderizarSecciones(currentLanguage); 
        });
    }
    ...
    // --- CONFIGURACIÓN DE LOS BOTONES DEL POP-UP MIXTA ---
    const btnSiPopup = document.getElementById("popup-btn-si");
    if (btnSiPopup) {
        btnSiPopup.addEventListener("click", () => {
            if (pantallaMixtaPendiente) {
                window.location.href = `${pantallaMixtaPendiente}.html`;
            }
        });
    }

    const btnNoPopup = document.getElementById("popup-btn-no");
    if (btnNoPopup) {
        btnNoPopup.addEventListener("click", () => {
            const modal = document.getElementById("popup-mixta");
            if (modal) modal.style.display = "none";
            pantallaMixtaPendiente = "";
        });
    }
    ...
    onSnapshot(qUsuario, async (querySnapshot) => {
        if (!querySnapshot.empty) {
            ...
            // Sincronizar elementos UI de la cabecera
            ...
            // --- PROCESADO Y GUARDADO DE EDAD LITERAL (int64) EN FIRESTORE ---
            if (userData.fechanacimiento) {
                const edadCalculada = calcularEdadCompleta(userData.fechanacimiento);
                
                // Si la edad guardada en la BD difiere del cálculo en tiempo real, actualizamos Firestore
            ...
            if (userData.subs && typeof userData.subs === "object") {
                ...
                if (userData.subs.fechaExpiracion) {
                    // Tomamos el Timestamp de Firebase expiración
                    ...
                    // Cálculo matemático exacto de la diferencia en días
                    ...
                    if (diasCalculados <= 0) {
                        // Si llega a 0 o menos, borramos el mapa subs completo de la base de datos
                        subsUsuarioGlobal = "";
                        ...
                        // Si los días calculados difieren de los guardados en BD, actualizamos
            ...
            // Forzar repintado si las secciones ya se cargaron
            if (listaSeccionesFirebase.length > 0) {
                renderizarSecciones(currentLanguage);
            }
        }
    }, (error) => {
    ...
    // 2. ESCUCHA EN TIEMPO REAL DE LAS SECCIONES (onSnapshot)
    const seccionesRef = collection(db, "Secciones");
    
    onSnapshot(seccionesRef, (snapshotSecciones) => {
        listaSeccionesFirebase = snapshotSecciones.docs;
        renderizarSecciones(currentLanguage);
    }, (error) => {
        console.error("Error en tiempo real con las secciones:", error);
    });
