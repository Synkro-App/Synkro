...
const traduccionesLocales = {
    ...
    "titulo_dificil": { "es": "Corre Dino - Difícil", "en": "Dino Run - Hard", "fr": "Course Dino - Difficile", "ro": "Fugi Dino - Dificil" }
};
...
function aplicarTraduccionesEstaticas() {
    ...
    const tituloDificil = document.getElementById("titulo-dificil");
    if (tituloDificil) tituloDificil.textContent = obtenerTextoTraduccion("titulo_dificil", "Corre Dino - Difícil");
}

// --- LÓGICA DEL JUEGO: DINO RUN (MODO DIFÍCIL) ---
function inicializarJuegoDino() {
    ...
    let totalAgachos = 0;
    let totalMonedas = 0;
    let gameSpeed = 9; // Velocidad elevada para el modo difícil
    ...
    const dino = {
        ...
        height: 40,
        vy: 0,
        gravity: 0.6, // Gravedad ligeramente mayor para caídas más ágiles
        jumpPower: -10,
        isJumping: false,
        isDuck: false,
        duckTimer: null,
        draw() {
            ctx.fillStyle = "#b71c1c"; // Tono rojizo distintivo para el modo difícil
            ...
            if (this.isDuck) {
                ...
                if (this.isJumping) {
                    ...
                    const animPata = Math.floor(Date.now() / 80) % 2; // Animación un poco más rápida
                    if (animPata === 0) {
        ...
        duck() {
            if (!this.isJumping) {
                ...
                this.duckTimer = setTimeout(() => {
                    this.standUp();
                }, 400); // Tiempo de agacho ligeramente más corto por la alta velocidad
            }
        },
        standUp() {
            this.isDuck = false;
    ...
    class Obstacle {
        constructor() {
            this.tipo = Math.random() > 0.45 ? "alto" : "suelo";
            if (this.tipo === "suelo") {
                this.width = 18;
                this.height = 32;
                this.x = canvas.width;
        ...
        draw() {
            ctx.fillStyle = "#880e4f"; // Obstáculos de color vino oscuro para dificultad alta
            if (this.tipo === "suelo") {
                ctx.beginPath();
    ...
    function spawnElementos() {
        ...
        } else if (probabilidad < 0.85 && tiempoDesdeUltimaMoneda >= 2) {
        ...
        // Intervalos de aparición más cortos y desafiantes
        const randomTime = Math.random() * 600 + 900;
    ...
    async function finalizarPartidaYGuardarRecompensas() {
        ...
        try {
            ...
            // Documento de transacción con el campo modo: "difícil" incluido
    ...
    function loop() {
        ...
        for (let i = obstacles.length - 1; i >= 0; i--) {
            ...
            if (obstacles[i].x + obstacles[i].width < 0) {
                ...
                if (Math.floor(distanciaMetros) % 40 === 0) gameSpeed += 0.2; // Aceleración más pronunciada en difícil
            }
        }
