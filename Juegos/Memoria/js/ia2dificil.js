...
// 15 iconos para cubrir las 15 parejas
...
const traduccionesLocales = {
    ...
    "titulo_ia2_dificil": { "es": "Memoria - Contra 2 IAs (Difícil)", "en": "Memory - Against 2 AIs (Hard)", "fr": "Mémoire - Contre 2 IAs (Difficile)", "ro": "Memorie - Contra 2 IA (Greu)" },
...
function aplicarTraduccionesEstaticas() {
    ...
    const tituloIa2Dificil = document.getElementById("titulo-ia2-dificil");
    if (tituloIa2Dificil) tituloIa2Dificil.textContent = obtenerTextoTraduccion("titulo_ia2_dificil", "Memoria - Contra 2 IAs (Difícil)");
...
// Memoria absoluta compartida para ambas IAs en nivel difícil
let memoriaIA = {}; 
...
function registrarMemoriaAbsoluta(carta) {
...
function voltearCartaUsuario(carta) {
    ...
    registrarMemoriaAbsoluta(carta);
...
function ejecutarTurnoIA(esIA2 = false) {
    ...
    // Memoria absoluta: busca inmediatamente si hay una pareja ya conocida
    ...
    // Si no hay pareja conocida completa, elige una al azar y busca su coincidencia en memoria
    if (!carta1) {
        cartasDisponibles.sort(() => 0.5 - Math.random());
        carta1 = cartasDisponibles[0];

        const iconoBuscado = carta1.dataset.icono;
        const indiceCoincidente = Object.keys(memoriaIA).find(idx => memoriaIA[idx] === iconoBuscado && idx !== carta1.dataset.indice);
...
async function finalizarPartida() {
    ...
    // 20 NovaPoints por cada pareja encontrada por el usuario en dificultad difícil
