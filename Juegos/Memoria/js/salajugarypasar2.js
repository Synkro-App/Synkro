...
const traduccionesLocales = {
    ...
    "titulo_juego": { "es": "Jugar y Pasar - 15 Parejas (3 Jugadores)", "en": "Play and Pass - 15 Pairs (3 Players)", "fr": "Jouer et Passer - 15 Paires (3 Joueurs)", "ro": "Joacă și Predă - 15 Perechi (3 Jucători)" }
};
...
function aplicarTraduccionesEstaticas() {
    ...
    if (tituloJuego) tituloJuego.textContent = obtenerTextoTraduccion("titulo_juego", "Jugar y Pasar - 15 Parejas (3 Jugadores)");
...
function inicializarJuego() {
    ...
    tablero.className = "grid-parejas";
    
    jugadorActual = 1;
    puntuaciones = { 1: 0, 2: 0, 3: 0 };
    cartasSeleccionadas = [];
    bloqueadoTablero = false;
    parejasEncontradasTotal = 0;

    infoTurno.textContent = `Turno del Jugador ${jugadorActual} (P1: ${puntuaciones[1]} | P2: ${puntuaciones[2]} | P3: ${puntuaciones[3]})`;
...
function verificarPareja() {
    ...
    if (esPareja) {
        ...
        const infoTurno = document.getElementById("info-turno");
        infoTurno.textContent = `¡Acertó J${jugadorActual}! Sigue su turno (P1: ${puntuaciones[1]} | P2: ${puntuaciones[2]} | P3: ${puntuaciones[3]})`;

        if (parejasEncontradasTotal === 15) {
            finalizarPartida();
        }
    } else {
        ...
        setTimeout(() => {
            ...
            jugadorActual = jugadorActual === 1 ? 2 : (jugadorActual === 2 ? 3 : 1);
            
            const infoTurno = document.getElementById("info-turno");
            infoTurno.textContent = `¡Pasa el dispositivo! Turno del Jugador ${jugadorActual} (P1: ${puntuaciones[1]} | P2: ${puntuaciones[2]} | P3: ${puntuaciones[3]})`;
...
function finalizarPartida() {
    ...
    if (p1 > p2 && p1 > p3) {
        textoGanador.textContent = `¡Gana el Jugador 1! (${p1} - ${p2} - ${p3})`;
    } else if (p2 > p1 && p2 > p3) {
        textoGanador.textContent = `¡Gana el Jugador 2! (${p2} - ${p1} - ${p3})`;
    } else if (p3 > p1 && p3 > p2) {
        textoGanador.textContent = `¡Gana el Jugador 3! (${p3} - ${p1} - ${p2})`;
    } else {
        textoGanador.textContent = `¡Empate! (${p1} - ${p2} - ${p3})`;
    }
}
