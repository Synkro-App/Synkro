...
const traduccionesLocales = {
    ...
    "titulo_juego": { "es": "Jugar y Pasar - 20 Parejas (4 Jugadores)", "en": "Play and Pass - 20 Pairs (4 Players)", "fr": "Jouer et Passer - 20 Paires (4 Joueurs)", "ro": "Joacă și Predă - 20 Perechi (4 Jucători)" },
...
function aplicarTraduccionesEstaticas() {
    ...
    if (tituloJuego) tituloJuego.textContent = obtenerTextoTraduccion("titulo_juego", "Jugar y Pasar - 20 Parejas (4 Jugadores)");
...
function inicializarJuego() {
    ...
    infoTurno.textContent = `Turno del Jugador ${jugadorActual} (P1:${puntuaciones[1]} | P2:${puntuaciones[2]} | P3:${puntuaciones[3]} | P4:${puntuaciones[4]})`;
...
function verificarPareja() {
    ...
    if (esPareja) {
        ...
        const infoTurno = document.getElementById("info-turno");
        infoTurno.textContent = `¡Acertó J${jugadorActual}! Sigue su turno (P1:${puntuaciones[1]} | P2:${puntuaciones[2]} | P3:${puntuaciones[3]} | P4:${puntuaciones[4]})`;

        if (parejasEncontradasTotal === 20) {
            finalizarPartida();
        }
    } else {
        ...
        setTimeout(() => {
            ...
            jugadorActual = jugadorActual === 1 ? 2 : (jugadorActual === 2 ? 3 : (jugadorActual === 3 ? 4 : 1));
            
            const infoTurno = document.getElementById("info-turno");
            infoTurno.textContent = `¡Pasa el dispositivo! Turno del Jugador ${jugadorActual} (P1:${puntuaciones[1]} | P2:${puntuaciones[2]} | P3:${puntuaciones[3]} | P4:${puntuaciones[4]})`;
...
function finalizarPartida() {
    ...
    const maxPunt = Math.max(p1, p2, p3, p4);
    ...
    if (ganadores.length === 1) {
        textoGanador.textContent = `¡Gana el ${ganadores[0]}! (${p1} - ${p2} - ${p3} - ${p4})`;
    } else {
        textoGanador.textContent = `¡Empate entre ${ganadores.join(", ")}! (${p1} - ${p2} - ${p3} - ${p4})`;
    }
}
