...
function aplicarTraduccionesEstaticas() {
    ...
    if (tituloIa1Dificil) tituloIa1Dificil.textContent = obtenerTextoTraduccion("titulo_ia1_dificil", "Memoria - Multijugador (4 Jugadores)");
}
...
async function actualizarCabeceraJugadores(data) {
    ...
    if (txtJ2) txtJ2.textContent = j2Name ? infoJ2.nombre : "Esperando...";

    if (imgJ3) imgJ3.src = infoJ3.avatar;
    if (txtJ3) txtJ3.textContent = j3Name ? infoJ3.nombre : "Esperando...";
...
async function gestionarMatchmaking() {
    ...
    snapshot.forEach(docSnap => {
        if (docSnap.id.startsWith("3aleatorios")) {
            docsValidos.push({ id: docSnap.id, data: docSnap.data() });
        }
    });

    docsValidos.sort((a, b) => {
        const numA = parseInt(a.id.replace("3aleatorios", ""), 10) || 0;
        const numB = parseInt(b.id.replace("3aleatorios", ""), 10) || 0;
        return numA - numB;
    });
    ...
    for (let item of docsValidos) {
        const d = item.data;
        if (d.jugador1 !== nombreUsuarioLogueado && d.jugador2 !== nombreUsuarioLogueado && d.jugador3 !== nombreUsuarioLogueado && d.jugador4 !== nombreUsuarioLogueado) {
    ...
    if (salaEncontrada) {
        ...
        partidaIdGlobal = `3aleatorios${nuevoNsala}`;
        rolJugador = "jugador1";
...
async function comprobarParejaRed() {
    ...
    else if (turnoActual === "jugador4") siguienteTurno = "jugador1";
    ...
    if (esCoincidencia) {
        ...
        const totalParejasEncontradas = p1 + p2 + p3 + p4;
        if (totalParejasEncontradas === totalParejas) {
            await actualizarFinPartidaDB(p1, p2, p3, p4, nuevoEstado);
            return;
        }
...
async function actualizarFinPartidaDB(p1, p2, p3, p4, estadoTablero) {
...
function escucharPartida() {
    unsubPartida = onSnapshot(doc(db, "Partidas de Memoria", partidaIdGlobal), async (docSnap) => {
        ...
        const partidaCompleta = data.jugador1 && data.jugador2 && data.jugador3 && data.jugador4;
        ...
        if (data.partida === "espera") {
        ...
        } else if (data.partida === "terminada" || ((data.parejas1 || 0) + (data.parejas2 || 0) + (data.parejas3 || 0) + (data.parejas4 || 0) === totalParejas)) {
            juegoTerminadoLocal = true;
            ...
            let textoModal = `Fin de partida - J1: ${p1} | J2: ${p2} | J3: ${p3} | J4: ${p4}`;
...
function mostrarPopupSolicitudReinicio(votosCount) {
    ...
    p.textContent = `Solicitud de revancha (${votosCount}/4 jugadores listos). ¿Deseas aceptar?`;
    btns.innerHTML = "";
