...
const traduccionesLocales = {
    ...
    "titulo_ia4_dificil": { "es": "Memoria - Contra 4 IAs (Difícil)", "en": "Memory - Against 4 AIs (Hard)", "fr": "Mémoire - Contre 4 IAs (Difficile)", "ro": "Memorie - Contra 4 IA (Greu)" },
...
function aplicarTraduccionesEstaticas() {
    ...
    const tituloIa4Dificil = document.getElementById("titulo-ia4-dificil");
    if (tituloIa4Dificil) tituloIa4Dificil.textContent = obtenerTextoTraduccion("titulo_ia4_dificil", "Memoria - Contra 4 IAs (Difícil)");
...
// Memoria absoluta para cada una de las 4 IAs (modo difícil)
let memoriaIA1 = {}; 
let memoriaIA2 = {}; 
let memoriaIA3 = {}; 
let memoriaIA4 = {}; 
...
function registrarMemoriaAbsolutaTodas(carta) {
    const indice = carta.dataset.indice;
    const icono = carta.dataset.icono;
    memoriaIA1[indice] = icono;
    memoriaIA2[indice] = icono;
    memoriaIA3[indice] = icono;
    memoriaIA4[indice] = icono;
}
...
function voltearCartaUsuario(carta) {
    ...
    registrarMemoriaAbsolutaTodas(carta);
...
function comprobarParejaUsuario() {
    ...
    if (esCoincidencia) {
        ...
        delete memoriaIA1[primeraCarta.dataset.indice]; delete memoriaIA1[segundaCarta.dataset.indice];
        delete memoriaIA2[primeraCarta.dataset.indice]; delete memoriaIA2[segundaCarta.dataset.indice];
        delete memoriaIA3[primeraCarta.dataset.indice]; delete memoriaIA3[segundaCarta.dataset.indice];
        delete memoriaIA4[primeraCarta.dataset.indice]; delete memoriaIA4[segundaCarta.dataset.indice];
...
function ejecutarTurnoIA() {
    ...
    if (!carta1) {
        ...
        const indiceCoincidente = Object.keys(memoriaActual).find(idx => memoriaActual[idx] === iconoBuscado && idx !== carta1.dataset.indice);

        if (indiceCoincidente) {
            const c2 = cartasDisponibles.find(c => c.dataset.indice === indiceCoincidente);
            if (c2) {
                carta2 = c2;
            }
        }
        ...
        if (!carta2) {
            const otrasCartas = cartasDisponibles.filter(c => c !== carta1);
            carta2 = otrasCartas[0];
        }
    }

    carta1.classList.add("flipped");
    carta1.textContent = carta1.dataset.icono;
    registrarMemoriaAbsolutaTodas(carta1);

    setTimeout(() => {
        if (juegoTerminado) return;
        carta2.classList.add("flipped");
        carta2.textContent = carta2.dataset.icono;
        registrarMemoriaAbsolutaTodas(carta2);
...
async function finalizarPartida() {
    ...
    // 20 NovaPoints por cada pareja encontrada por el usuario en modo difícil
    const totalNovaPointsGanados = parejasUsuario * 20;
