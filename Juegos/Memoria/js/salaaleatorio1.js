...
const traduccionesLocales = {
    ...
    "esperando_oponente": { "es": "Esperando oponente...", "en": "Waiting for opponent...", "fr": "En attente d'un adversaire...", "ro": "Se așteaptă oponentul..." },
...
async function actualizarCabeceraJugadores(data) {
    ...
    if (txtJ2) txtJ2.textContent = j2Name ? `J2: ${infoJ2.nombre}` : obtenerTextoTraduccion("esperando_oponente", "Esperando oponente...");
}
...
async function gestionarMatchmaking() {
    ...
    snapshot.forEach(docSnap => {
        if (docSnap.id.startsWith("1aleatorio")) {
            docsValidos.push({ id: docSnap.id, data: docSnap.data() });
        }
    });

    docsValidos.sort((a, b) => {
        const numA = parseInt(a.id.replace("1aleatorio", ""), 10) || 0;
        const numB = parseInt(b.id.replace("1aleatorio", ""), 10) || 0;
        return numA - numB;
    });

    let salaEncontrada = null;
    for (let item of docsValidos) {
        if (!item.data.jugador2 || item.data.jugador2 === "") {
            if (item.data.jugador1 !== nombreUsuarioLogueado) {
                salaEncontrada = item;
                break;
            }
        }
    ...
    if (salaEncontrada) {
        partidaIdGlobal = salaEncontrada.id;
        rolJugador = "jugador2";
        ...
        partidaIdGlobal = `1aleatorio${nuevoNsala}`;
        rolJugador = "jugador1";
    ...
    cartasArray.forEach((icono, index) => {
        ...
        if (estadoTablero && estadoTablero[index]) {
            if (estadoTablero[index].matched) {
                ...
                tarjeta.textContent = icono; // Se mantiene el emoji visible al estar encontradas
...
async function comprobarParejaRed() {
    ...
    let siguienteTurno = datosPartidaGlobal.turno;
    ...
    if (esCoincidencia) {
        ...
        if (p1 + p2 === totalParejas) {
            await actualizarFinPartidaDB(p1, p2, nuevoEstado);
            return;
        }
    } else {
        ...
        siguienteTurno = (rolJugador === "jugador1") ? "jugador2" : "jugador1";
        resetearSeleccion();
        bloqueoTablero = false;
    }
...
async function actualizarFinPartidaDB(p1, p2, estadoTablero) {
    ...
    unsubPartida = onSnapshot(doc(db, "Partidas de Memoria", partidaIdGlobal), async (docSnap) => {
        ...
        if (data.partida === "espera") {
            if (!data.jugador2 || data.jugador2 === "") {
                if (indicador) indicador.textContent = obtenerTextoTraduccion("esperando_oponente", "Esperando oponente...");
            } else {
                if (indicador) {
                    if (data.turno === rolJugador) {
                        ...
                        const oponenteNombre = rolJugador === "jugador1" ? data.jugador2 : data.jugador1;
        ...
        } else if (data.partida === "terminada" || data.parejas1 + data.parejas2 === totalParejas) {
            juegoTerminadoLocal = true;
            const p1 = data.parejas1 || 0;
            const p2 = data.parejas2 || 0;
            
            let parejasMis = (rolJugador === "jugador1") ? p1 : p2;
            ...
            let textoModal = "";
            if (p1 > p2) {
                const ganadorNombre = data.jugador1;
                textoModal = p1 === p2 ? obtenerTextoTraduccion("partida_empatada", "Partida empatada") : `Ganador Jugador 1: ${ganadorNombre}`;
            } else if (p2 > p1) {
                textoModal = `Ganador Jugador 2: ${data.jugador2}`;
            } else {
                textoModal = obtenerTextoTraduccion("partida_empatada", "Partida empatada");
            }

            mostrarPopupGanador(textoModal, data);
        }

        if (data.peticion === "espera") {
            const esQuienPide = data.peticionSolicitante === rolJugador;
            if (esQuienPide) {
                mostrarPopupEsperaRespuesta();
            } else {
                const nombreSolicitante = rolJugador === "jugador1" ? data.jugador2 : data.jugador1;
                const numSolicitante = rolJugador === "jugador1" ? "2" : "1";
                mostrarPopupSolicitudReinicio(numSolicitante, nombreSolicitante);
            }
        } else if (data.peticion === "rechazada") {
            const esQuienPide = data.peticionSolicitante === rolJugador;
            if (esQuienPide) {
                const nombreOtro = rolJugador === "jugador1" ? data.jugador2 : data.jugador1;
                const numOtro = rolJugador === "jugador1" ? "2" : "1";
                mostrarPopupAvisoSalida(`Jugador ${numOtro}: ${nombreOtro} ${obtenerTextoTraduccion("ya_no_quiere_jugar", "ya no quiere o puede jugar más")}`);
            }
        } else if (data.peticion === "" && data.partida === "espera" && juegoTerminadoLocal) {
            // Se acepta la revancha, limpiamos popups de espera para ambos
...
function mostrarPopupGanador(texto, data) {
    const modal = document.getElementById("modal-popup");
    ...
    btnReintentar.addEventListener("click", async () => {
        await updateDoc(doc(db, "Partidas de Memoria", partidaIdGlobal), {
            peticion: "espera",
            peticionSolicitante: rolJugador
        });
        mostrarPopupEsperaRespuesta();
    });
...
function mostrarPopupEsperaRespuesta() {
    ...
    p.textContent = obtenerTextoTraduccion("esperando_respuesta", "Esperando respuesta");
    btns.innerHTML = "";
    modal.classList.remove("hidden");
}

function mostrarPopupSolicitudReinicio(numSolicitante, nombreSolicitante) {
    const modal = document.getElementById("modal-popup");
    const p = document.getElementById("modal-text");
    const btns = document.getElementById("modal-buttons");

    p.textContent = `Jugador ${numSolicitante}: ${nombreSolicitante} ${obtenerTextoTraduccion("quiere_volver_a_jugar", "quiere volver a jugar")}`;
    btns.innerHTML = "";
