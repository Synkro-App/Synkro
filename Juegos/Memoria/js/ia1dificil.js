...
const traduccionesLocales = {
    ...
    "titulo_ia1_dificil": { "es": "Memoria - Contra IA (Difícil)", "en": "Memory - Against AI (Hard)", "fr": "Mémoire - Contre IA (Difficile)", "ro": "Memorie - Contra IA (Greu)" },
...
function aplicarTraduccionesEstaticas() {
    ...
    if (tituloIa1Dificil) tituloIa1Dificil.textContent = obtenerTextoTraduccion("titulo_ia1_dificil", "Memoria - Contra IA (Difícil)");
...
// Memoria absoluta de la IA (guarda permanentemente qué icono hay en cada tarjeta revelada)
let memoriaIA = {}; 
...
function registrarMemoriaAbsoluta(carta) {
    ...
    // Memoria absoluta: recuerda siempre cualquier carta que se muestre en el tablero
...
function turnoIA() {
    ...
    // Con memoria absoluta, busca inmediatamente si hay una pareja ya conocida en memoria
    ...
    // Si no hay una pareja completa conocida, elige una carta al azar y busca su pareja en memoria si existe
    if (!carta1) {
        ...
        if (!carta2) {
            // Si la pareja no se conoce, elige otra carta disponible al azar
...
async function finalizarPartida() {
    ...
    // 20 NovaPoints por cada pareja encontrada por el usuario
