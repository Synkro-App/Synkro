...
const traduccionesLocales = {
    ...
    "titulo_modo_tiempo": { "es": "Selecciona modo contrarreloj", "en": "Select time attack mode", "fr": "Sélectionner le mode contre la montre", "ro": "Selectează modul contra timp" },
    "text_btn_facil": { "es": "Fácil - 5 minutos (10 parejas)", "en": "Easy - 5 minutes (10 pairs)", "fr": "Facile - 5 minutes (10 paires)", "ro": "Ușor - 5 minute (10 perechi)" },
    "text_btn_medio": { "es": "Medio - 2 minutos (10 parejas)", "en": "Medium - 2 minutes (10 pairs)", "fr": "Moyen - 2 minutes (10 paires)", "ro": "Mediu - 2 minute (10 perechi)" },
    "text_btn_dificil": { "es": "Difícil - 1 minuto (10 parejas)", "en": "Hard - 1 minute (10 pairs)", "fr": "Difficile - 1 minute (10 paires)", "ro": "Greu - 1 minut (10 perechi)" }
};
...
function aplicarTraduccionesEstaticas() {
    ...
    const tituloModoTiempo = document.getElementById("titulo-modo-tiempo");
    if (tituloModoTiempo) tituloModoTiempo.textContent = obtenerTextoTraduccion("titulo_modo_tiempo", "Selecciona modo contrarreloj");

    const textBtnFacil = document.getElementById("text-btn-facil");
    if (textBtnFacil) textBtnFacil.textContent = obtenerTextoTraduccion("text_btn_facil", "Fácil - 5 minutos (10 parejas)");

    const textBtnMedio = document.getElementById("text-btn-medio");
    if (textBtnMedio) textBtnMedio.textContent = obtenerTextoTraduccion("text_btn_medio", "Medio - 2 minutos (10 parejas)");

    const textBtnDificil = document.getElementById("text-btn-dificil");
    if (textBtnDificil) textBtnDificil.textContent = obtenerTextoTraduccion("text_btn_dificil", "Difícil - 1 minuto (10 parejas)");
}
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    // Configuración de redirección de los botones de los modos
    const btnFacil5m = document.getElementById("btn-facil-5m");
    if (btnFacil5m) {
        btnFacil5m.addEventListener("click", () => { window.location.href = "solofacil5m.html"; });
    }

    const btnMedio2m = document.getElementById("btn-medio-2m");
    if (btnMedio2m) {
        btnMedio2m.addEventListener("click", () => { window.location.href = "solomedio2m.html"; });
    }

    const btnDificil1m = document.getElementById("btn-dificil-1m");
    if (btnDificil1m) {
        btnDificil1m.addEventListener("click", () => { window.location.href = "solodificil1m.html"; });
    }
    ...
    try {
        ...
        if (!querySnapshot.empty) {
            ...
            const novasActuales = userData.NovaPoints !== undefined ? userData.NovaPoints : 0;
