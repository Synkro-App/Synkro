...
const traduccionesLocales = {
    ...
    "titulo_ia1_medio": { "es": "Memoria - Contra IA (Medio)", "en": "Memory - Against AI (Medium)", "fr": "Mémoire - Contre IA (Moyen)", "ro": "Memorie - Contra IA (Mediu)" },
    ...
    "turno_ia": { "es": "Turno de la IA...", "en": "AI's turn...", "fr": "Tour de l'IA...", "ro": "Rândul IA..." }
};
...
function aplicarTraduccionesEstaticas() {
    ...
    const tituloIa1Medio = document.getElementById("titulo-ia1-medio");
    if (tituloIa1Medio) tituloIa1Medio.textContent = obtenerTextoTraduccion("titulo_ia1_medio", "Memoria - Contra IA (Medio)");
...
// Memoria de la IA (mapa para recordar qué icono hay en cada tarjeta vista recientemente)
let memoriaIA = {}; 
...
function actualizarIndicadorTurno() {
    ...
    if (turnoActual === "usuario") {
        ...
        indicador.textContent = obtenerTextoTraduccion("turno_ia", "Turno de la IA...");
...
function registrarMemoria(carta) {
    ...
    // Probabilidad moderada (70%) de que la IA recuerde la carta que ve
    if (Math.random() < 0.7) {
...
function comprobarParejaUsuario() {
    ...
    if (esCoincidencia) {
        ...
        if (parejasUsuario + parejasIA === totalParejas) {
            finalizarPartida();
        } else {
            actualizarIndicadorTurno();
        }
...
function turnoIA() {
    ...
    // Buscar en la memoria si la IA conoce una pareja disponible
    ...
    // Si no encontró una pareja completa en memoria, juega de forma mixta (nivel medio)
    if (!carta1) {
        cartasDisponibles.sort(() => 0.5 - Math.random());
        carta1 = cartasDisponibles[0];

        // Intentar buscar pareja para carta1 usando la memoria
...
async function finalizarPartida() {
    ...
    // 10 NovaPoints por cada pareja encontrada por el usuario
    ...
    let textoResultado = "";
    if (parejasUsuario > parejasIA) {
        textoResultado = currentLanguage === "en" ? `You won! You: ${parejasUsuario} - AI: ${parejasIA}` : `¡Has ganado! Usuario: ${parejasUsuario} - IA: ${parejasIA}`;
    } else if (parejasUsuario < parejasIA) {
        textoResultado = currentLanguage === "en" ? `AI won. You: ${parejasUsuario} - AI: ${parejasIA}` : `Gana la IA. Usuario: ${parejasUsuario} - IA: ${parejasIA}`;
    } else {
        textoResultado = currentLanguage === "en" ? `Empate. You: ${parejasUsuario} - AI: ${parejasIA}` : `Empate. Usuario: ${parejasUsuario} - IA: ${parejasIA}`;
    }
