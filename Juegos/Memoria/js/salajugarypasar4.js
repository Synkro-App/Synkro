...
const traduccionesLocales = {
    ...
    "titulo_juego": { "es": "Jugar y Pasar - 40 Parejas (5 Jugadores)", "en": "Play and Pass - 40 Pairs (5 Players)", "fr": "Jouer et Passer - 40 Paires (5 Joueurs)", "ro": "Joacă și Predă - 40 Perechi (5 Jucători)" },
    "btn_reiniciar": { "es": "Jugar de nuevo", "en": "Play again", "fr": "Rejouer", "ro": "Joacă din nou" }
};
...
function aplicarTraduccionesEstaticas() {
    ...
    if (tituloJuego) tituloJuego.textContent = obtenerTextoTraduccion("titulo_juego", "Jugar y Pasar - 40 Parejas (5 Jugadores)");
...
function inicializarJuego() {
    ...
    infoTurno.textContent = `Turno del Jugador ${jugadorActual} (P1:${puntuaciones[1]} | P2:${puntuaciones[2]} | P3:${puntuaciones[3]} | P4:${puntuaciones[4]} | P5:${puntuaciones[5]})`;
...
function verificarPareja() {
    ...
    if (esPareja) {
        ...
        const infoTurno = document.getElementById("info-turno");
        infoTurno.textContent = `¡Acertó J${jugadorActual}! Sigue su turno (P1:${puntuaciones[1]} | P2:${puntuaciones[2]} | P3:${puntuaciones[3]} | P4:${puntuaciones[4]} | P5:${puntuaciones[5]})`;

        if (parejasEncontradasTotal === 40) {
            finalizarPartida();
        }
    } else {
        ...
        setTimeout(() => {
            ...
            jugadorActual = jugadorActual === 1 ? 2 : (jugadorActual === 2 ? 3 : (jugadorActual === 3 ? 4 : (jugadorActual === 4 ? 5 : 1)));
            
            const infoTurno = document.getElementById("info-turno");
            infoTurno.textContent = `¡Pasa el dispositivo! Turno del Jugador ${jugadorActual} (P1:${puntuaciones[1]} | P2:${puntuaciones[2]} | P3:${puntuaciones[3]} | P4:${puntuaciones[4]} | P5:${puntuaciones[5]})`;
...
function finalizarPartida() {
    ...
    const p1 = puntuaciones[1];
    const p2 = puntuaciones[2];
    const p3 = puntuaciones[3];
    const p4 = puntuaciones[4];
    const p5 = puntuaciones[5];
    const maxPunt = Math.max(p1, p2, p3, p4, p5);

    let ganadores = [];
    if (p1 === maxPunt) ganadores.push("Jugador 1");
    if (p2 === maxPunt) ganadores.push("Jugador 2");
    if (p3 === maxPunt) ganadores.push("Jugador 3");
    if (p4 === maxPunt) ganadores.push("Jugador 4");
    if (p5 === maxPunt) ganadores.push("Jugador 5");

    if (ganadores.length === 1) {
        textoGanador.textContent = `¡Gana el ${ganadores[0]}! (${p1} - ${p2} - ${p3} - ${p4} - ${p5})`;
    } else {
        textoGanador.textContent = `¡Empate entre ${ganadores.join(", ")}! (${p1} - ${p2} - ${p3} - ${p4} - ${p5})`;
    }
}
