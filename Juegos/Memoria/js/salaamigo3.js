...
const traduccionesLocales = {
    ...
    "titulo_sala": { "es": "Sala Amigo - Memoria 4 Jugadores", "en": "Friend Room - 4 Players Memory", "fr": "Salle Amie - Mémoire 4 Joueurs", "ro": "Cameră Prieten - Memorie 4 Jucători" },
...
function aplicarTraduccionesEstaticas() {
    ...
    if (tituloSala) tituloSala.textContent = obtenerTextoTraduccion("titulo_sala", "Sala Amigo - Memoria 4 Jugadores");
...
async function renderizarCabeceraJugadores(data) {
    ...
    if (!contenedorCabecera) {
        ...
        contenedorCabecera.style.cssText = "display: flex; justify-content: space-around; align-items: center; margin-bottom: 15px; width: 100%; max-width: 550px; flex-wrap: wrap; gap: 10px;";
        const infoTurno = document.getElementById("info-turno");
    ...
    let img1 = "../../default-profile.png";
    let img2 = "../../default-profile.png";
    let img3 = "../../default-profile.png";
    let img4 = "../../default-profile.png";

    if (data.jugador1DocId) {
        if (data.jugador1DocId === userDocIdGlobal) img1 = avatarUsuarioGlobal;
        else {
            try {
                const snap = await getDoc(doc(db, "Usuarios", data.jugador1DocId));
                if (snap.exists()) img1 = normalizarSourceImagen(snap.data().imgperfil);
            } catch (e) {}
        }
    }

    if (data.jugador2DocId) {
        if (data.jugador2DocId === userDocIdGlobal) img2 = avatarUsuarioGlobal;
        else {
            try {
                const snap = await getDoc(doc(db, "Usuarios", data.jugador2DocId));
                if (snap.exists()) img2 = normalizarSourceImagen(snap.data().imgperfil);
            } catch (e) {}
        }
    }

    if (data.jugador3DocId) {
        if (data.jugador3DocId === userDocIdGlobal) img3 = avatarUsuarioGlobal;
        else {
            try {
                const snap = await getDoc(doc(db, "Usuarios", data.jugador3DocId));
                if (snap.exists()) img3 = normalizarSourceImagen(snap.data().imgperfil);
            } catch (e) {}
        }
    }

    if (data.jugador4DocId) {
        if (data.jugador4DocId === userDocIdGlobal) img4 = avatarUsuarioGlobal;
        else {
            try {
                const snap = await getDoc(doc(db, "Usuarios", data.jugador4DocId));
                if (snap.exists()) img4 = normalizarSourceImagen(snap.data().imgperfil);
            } catch (e) {}
        }
    }

    const nom1 = data.jugador1 || "J1";
    const nom2 = data.jugador2 || "J2";
    const nom3 = data.jugador3 || "J3";
    const nom4 = data.jugador4 || "J4";

    contenedorCabecera.innerHTML = `
        <div style="display: flex; align-items: center; gap: 4px;">
            <img src="${img1}" style="width: 30px; height: 30px; border-radius: 50%; object-fit: cover; border: 2px solid ${jugadorActualTurno === 1 ? '#4e54c8' : 'transparent'};" />
            <span style="font-size: 0.75rem; font-weight: bold;">J1: ${nom1}</span>
        </div>
        <div style="display: flex; align-items: center; gap: 4px;">
            <img src="${img2}" style="width: 30px; height: 30px; border-radius: 50%; object-fit: cover; border: 2px solid ${jugadorActualTurno === 2 ? '#4e54c8' : 'transparent'};" />
            <span style="font-size: 0.75rem; font-weight: bold;">J2: ${nom2}</span>
        </div>
        <div style="display: flex; align-items: center; gap: 4px;">
            <img src="${img3}" style="width: 30px; height: 30px; border-radius: 50%; object-fit: cover; border: 2px solid ${jugadorActualTurno === 3 ? '#4e54c8' : 'transparent'};" />
            <span style="font-size: 0.75rem; font-weight: bold;">J3: ${nom3}</span>
        </div>
        <div style="display: flex; align-items: center; gap: 4px;">
            <img src="${img4}" style="width: 30px; height: 30px; border-radius: 50%; object-fit: cover; border: 2px solid ${jugadorActualTurno === 4 ? '#4e54c8' : 'transparent'};" />
            <span style="font-size: 0.75rem; font-weight: bold;">J4: ${nom4}</span>
        </div>
    `;
}
...
async function actualizarVisualesTablero(data) {
    ...
    if (infoTurno) {
        ...
        const textoTuTurno = obtenerTextoTraduccion("tu_turno", "¡Es tu turno!");
        const textoTurnoJugador = obtenerTextoTraduccion("turno_jugador", "Turno del Jugador");
        infoTurno.textContent = `${soyTurno ? textoTuTurno : textoTurnoJugador + ' ' + jugadorActualTurno} (P1: ${puntuaciones[1]} | P2: ${puntuaciones[2]} | P3: ${puntuaciones[3]} | P4: ${puntuaciones[4]})`;
    }
...
function escucharPartida(partidaId) {
    ...
    unsubPartida = onSnapshot(partidaRef, async (docSnap) => {
        ...
        const salaCompleta = data.jugador1 && data.jugador2 && data.jugador3 && data.jugador4;
        ...
        if ((salaCompleta && (menuAmigoVisible || contenedorJuegoOculto)) && data.partida !== "fin" && data.partida !== "espera" && data.partida !== "terminada") {
            ...
            if (txtEsp) {
                ...
                txtEsp.textContent = `Esperando jugadores (${count}/4)...`;
            }
        }
        ...
        if (data.partida === "fin") {
            ...
            let mensajeResultado = `${data.ganadorTextoUltimo || "Partida finalizada"} (P1: ${p1} | P2: ${p2} | P3: ${p3} | P4: ${p4})`;
