...
let numeroJugadorAsignado = 0; // 1, 2 o 3
let codigoActualPartida = "";

// Estado de juego de memoria 3 jugadores sincronizado
let jugadorActualTurno = 1; 
let puntuaciones = { 1: 0, 2: 0, 3: 0 };
...
const traduccionesLocales = {
    ...
    "titulo_sala": { "es": "Sala Amigo - Memoria 3 Jugadores", "en": "Friend Room - 3 Players Memory", "fr": "Salle Amie - Mémoire 3 Joueurs", "ro": "Cameră Prieten - Memorie 3 Jucători" },
...
function aplicarTraduccionesEstaticas() {
    ...
    if (tituloSala) tituloSala.textContent = obtenerTextoTraduccion("titulo_sala", "Sala Amigo - Memoria 3 Jugadores");
...
async function generarIdPartidaAmigo() {
    ...
    snapshot.forEach(docSnap => {
        const id = docSnap.id;
        if (id.startsWith("sala2amigos")) {
            const numStr = id.replace("sala2amigos", "");
...
async function renderizarCabeceraJugadores(data) {
    ...
    const nom3 = data.jugador3 || "Jugador 3";

    contenedorCabecera.innerHTML = `
        <div style="display: flex; align-items: center; gap: 6px;">
            <img src="${img1}" style="width: 35px; height: 35px; border-radius: 50%; object-fit: cover; border: 2px solid ${jugadorActualTurno === 1 ? '#4e54c8' : 'transparent'};" />
            <span style="font-size: 0.85rem; font-weight: bold;">J1: ${nom1}</span>
        </div>
        <div style="display: flex; align-items: center; gap: 6px;">
            <img src="${img2}" style="width: 35px; height: 35px; border-radius: 50%; object-fit: cover; border: 2px solid ${jugadorActualTurno === 2 ? '#4e54c8' : 'transparent'};" />
            <span style="font-size: 0.85rem; font-weight: bold;">J2: ${nom2}</span>
        </div>
        <div style="display: flex; align-items: center; gap: 6px;">
            <img src="${img3}" style="width: 35px; height: 35px; border-radius: 50%; object-fit: cover; border: 2px solid ${jugadorActualTurno === 3 ? '#4e54c8' : 'transparent'};" />
            <span style="font-size: 0.85rem; font-weight: bold;">J3: ${nom3}</span>
        </div>
    `;
}
...
async function actualizarVisualesTablero(data) {
    ...
    if (infoTurno) {
        ...
        const textoTuTurno = obtenerTextoTraduccion("tu_turno", "¡Es tu turno!");
        const textoTurnoJugador = obtenerTextoTraduccion("turno_jugador", "Turno del Jugador");
        infoTurno.textContent = `${soyTurno ? textoTuTurno : textoTurnoJugador + ' ' + jugadorActualTurno} (P1: ${puntuaciones[1]} | P2: ${puntuaciones[2]} | P3: ${puntuaciones[3]})`;
    }
    ...
    unsubPartida = onSnapshot(partidaRef, async (docSnap) => {
        ...
        if ((salaCompleta && (menuAmigoVisible || contenedorJuegoOculto)) && data.partida !== "espera" && data.partida !== "terminada") {
            ...
            if (txtEsp) {
                ...
                txtEsp.textContent = `Esperando jugadores (${count}/3)...`;
            }
        }
