...
// Se amplían los iconos disponibles a 15 para cubrir las 15 parejas
...
const traduccionesLocales = {
    ...
    "titulo_ia2_medio": { "es": "Memoria - Contra 2 IAs (Medio)", "en": "Memory - Against 2 AIs (Medium)", "fr": "Mémoire - Contre 2 IAs (Moyen)", "ro": "Memorie - Contra 2 IA (Mediu)" },
...
function aplicarTraduccionesEstaticas() {
    ...
    const tituloIa2Medio = document.getElementById("titulo-ia2-medio");
    if (tituloIa2Medio) tituloIa2Medio.textContent = obtenerTextoTraduccion("titulo_ia2_medio", "Memoria - Contra 2 IAs (Medio)");
...
let turnoActual = "usuario"; // "usuario", "ia1", "ia2"

// Memoria compartida o independiente para las IAs en nivel medio
let memoriaIA = {}; 
...
function actualizarIndicadorTurno() {
    ...
    if (turnoActual === "usuario") {
        ...
        indicador.style.backgroundColor = "rgba(255, 180, 100, 0.3)";
    }
}
...
function registrarMemoria(carta) {
    const indice = carta.dataset.indice;
    const icono = carta.dataset.icono;
    // Probabilidad moderada (70%) de que las IAs recuerden la carta vista
    if (Math.random() < 0.7) {
        memoriaIA[indice] = icono;
    }
}
...
function comprobarParejaUsuario() {
    ...
    if (esCoincidencia) {
        ...
        delete memoriaIA[primeraCarta.dataset.indice];
        delete memoriaIA[segundaCarta.dataset.indice];
        resetearSeleccionCartas();
        parejasUsuario++;

        if (parejasUsuario + parejasIA1 + parejasIA2 === totalParejas) {
            finalizarPartida();
        } else {
            actualizarIndicadorTurno();
        }
...
function ejecutarTurnoIA(esIA2 = false) {
    ...
    // Buscar en la memoria si se conoce una pareja completa
    const entradasMemoria = Object.entries(memoriaIA);
    ...
    // Si no hay pareja completa en memoria, juega de forma mixta (nivel medio)
    if (!carta1) {
        cartasDisponibles.sort(() => 0.5 - Math.random());
        carta1 = cartasDisponibles[0];

        const iconoBuscado = carta1.dataset.icono;
        const indiceCoincidenteEnMemoria = Object.keys(memoriaIA).find(idx => memoriaIA[idx] === iconoBuscado && idx !== carta1.dataset.indice);
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            ...
            if (esCoincidencia) {
                ...
                delete memoriaIA[carta1.dataset.indice];
                delete memoriaIA[carta2.dataset.indice];
                ...
                if (!esIA2) {
                    // Pasa el turno de IA 1 a IA 2
                    turnoActual = "ia2";
                    actualizarIndicadorTurno();
                    setTimeout(turnoIA2, 1000);
                } else {
                    // Pasa el turno de IA 2 al Usuario
                    turnoActual = "usuario";
                    actualizarIndicadorTurno();
                }
            }
...
function turnoIA1() {
    ejecutarTurnoIA(false);
}

function turnoIA2() {
    ejecutarTurnoIA(true);
}

function verificarFinOContinuar() {
    if (parejasUsuario + parejasIA1 + parejasIA2 >= totalParejas) {
        finalizarPartida();
    } else {
        turnoActual = "usuario";
...
async function finalizarPartida() {
    ...
    // 10 NovaPoints por cada pareja encontrada por el usuario en nivel medio
    ...
    let textoResultado = "";
    const maxParejas = Math.max(parejasUsuario, parejasIA1, parejasIA2);
    
    if (parejasUsuario === maxParejas && parejasUsuario !== parejasIA1 && parejasUsuario !== parejasIA2) {
        textoResultado = currentLanguage === "en" ? `You won! You: ${parejasUsuario} - AI 1: ${parejasIA1} - AI 2: ${parejasIA2}` : `¡Has ganado! Usuario: ${parejasUsuario} - IA 1: ${parejasIA1} - IA 2: ${parejasIA2}`;
    } else if (parejasIA1 === maxParejas && parejasIA1 !== parejasUsuario && parejasIA1 !== parejasIA2) {
        textoResultado = currentLanguage === "en" ? `AI 1 won. You: ${parejasUsuario} - AI 1: ${parejasIA1} - AI 2: ${parejasIA2}` : `Gana la IA 1. Usuario: ${parejasUsuario} - IA 1: ${parejasIA1} - IA 2: ${parejasIA2}`;
    } else if (parejasIA2 === maxParejas && parejasIA2 !== parejasUsuario && parejasIA2 !== parejasIA1) {
        textoResultado = currentLanguage === "en" ? `AI 2 won. You: ${parejasUsuario} - AI 1: ${parejasIA1} - AI 2: ${parejasIA2}` : `Gana la IA 2. Usuario: ${parejasUsuario} - IA 1: ${parejasIA1} - IA 2: ${parejasIA2}`;
    } else {
        textoResultado = currentLanguage === "en" ? `Empate. You: ${parejasUsuario} - AI 1: ${parejasIA1} - AI 2: ${parejasIA2}` : `Empate. Usuario: ${parejasUsuario} - IA 1: ${parejasIA1} - IA 2: ${parejasIA2}`;
    }
