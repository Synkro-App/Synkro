...
const traduccionesLocales = {
    ...
    "titulo_ia4_medio": { "es": "Memoria - Contra 4 IAs (Medio)", "en": "Memory - Against 4 AIs (Medium)", "fr": "Mémoire - Contre 4 IAs (Moyen)", "ro": "Memorie - Contra 4 IA (Mediu)" },
...
function aplicarTraduccionesEstaticas() {
    ...
    if (btnVolver) btnVolver.textContent = obtenerTextoTraduccion("text_volver", "Volver");
    
    const btnLogout = document.getElementById("btn-logout");
    if (btnLogout) btnLogout.textContent = obtenerTextoTraduccion("btn_cerrar_sesion", "Cerrar sesión");
    
    const tituloIa4Medio = document.getElementById("titulo-ia4-medio");
    if (tituloIa4Medio) tituloIa4Medio.textContent = obtenerTextoTraduccion("titulo_ia4_medio", "Memoria - Contra 4 IAs (Medio)");
...
let turnoActual = "usuario"; // "usuario", "ia1", "ia2", "ia3", "ia4"

// Memorias individuales para cada IA
let memoriaIA1 = {}; 
let memoriaIA2 = {}; 
let memoriaIA3 = {}; 
let memoriaIA4 = {}; 
...
function registrarMemoriaTodas(carta) {
    const indice = carta.dataset.indice;
    const icono = carta.dataset.icono;
    if (Math.random() < 0.7) memoriaIA1[indice] = icono;
    if (Math.random() < 0.7) memoriaIA2[indice] = icono;
    if (Math.random() < 0.7) memoriaIA3[indice] = icono;
    if (Math.random() < 0.7) memoriaIA4[indice] = icono;
}
...
function comprobarParejaUsuario() {
    ...
    if (esCoincidencia) {
        ...
        delete memoriaIA1[primeraCarta.dataset.indice];
        delete memoriaIA1[segundaCarta.dataset.indice];
        delete memoriaIA2[primeraCarta.dataset.indice];
        delete memoriaIA2[segundaCarta.dataset.indice];
        delete memoriaIA3[primeraCarta.dataset.indice];
        delete memoriaIA3[segundaCarta.dataset.indice];
        delete memoriaIA4[primeraCarta.dataset.indice];
        delete memoriaIA4[segundaCarta.dataset.indice];
        
        resetearSeleccionCartas();
        parejasUsuario++;

        const totalEncontradas = parejasUsuario + parejasIA1 + parejasIA2 + parejasIA3 + parejasIA4;
        if (totalEncontradas === totalParejas) {
            finalizarPartida();
        } else {
            actualizarIndicadorTurno();
        }
...
function ejecutarTurnoIA() {
    ...
    if (cartasDisponibles.length < 2) {
        verificarFinJuegoGeneral();
        return;
    }

    let memoriaActual;
    let nombreIA = turnoActual;
    if (nombreIA === "ia1") memoriaActual = memoriaIA1;
    else if (nombreIA === "ia2") memoriaActual = memoriaIA2;
    else if (nombreIA === "ia3") memoriaActual = memoriaIA3;
    else memoriaActual = memoriaIA4;

    let carta1 = null;
    let carta2 = null;

    const entradasMemoria = Object.entries(memoriaActual);
    let parejaEncontradaEnMemoria = false;

    for (let i = 0; i < entradasMemoria.length; i++) {
        for (let j = i + 1; j < entradasMemoria.length; j++) {
            const [idx1, icono1] = entradasMemoria[i];
            const [idx2, icono2] = entradasMemoria[j];

            if (icono1 === icono2) {
                const c1 = cartasDisponibles.find(c => c.dataset.indice === idx1);
                const c2 = cartasDisponibles.find(c => c.dataset.indice === idx2);
                if (c1 && c2) {
                    carta1 = c1;
                    carta2 = c2;
                    parejaEncontradaEnMemoria = true;
                    break;
                }
            }
        }
        if (parejaEncontradaEnMemoria) break;
    }

    if (!carta1) {
        cartasDisponibles.sort(() => 0.5 - Math.random());
        carta1 = cartasDisponibles[0];

        const iconoBuscado = carta1.dataset.icono;
        const indiceCoincidenteEnMemoria = Object.keys(memoriaActual).find(idx => memoriaActual[idx] === iconoBuscado && idx !== carta1.dataset.indice);

        if (indiceCoincidenteEnMemoria) {
            const c2 = cartasDisponibles.find(c => c.dataset.indice === indiceCoincidenteEnMemoria);
            if (c2) {
                carta2 = c2;
            }
        }
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            ...
            if (esCoincidencia) {
                ...
                delete memoriaIA1[carta1.dataset.indice]; delete memoriaIA1[carta2.dataset.indice];
                delete memoriaIA2[carta1.dataset.indice]; delete memoriaIA2[carta2.dataset.indice];
                delete memoriaIA3[carta1.dataset.indice]; delete memoriaIA3[carta2.dataset.indice];
                delete memoriaIA4[carta1.dataset.indice]; delete memoriaIA4[carta2.dataset.indice];

                if (nombreIA === "ia1") parejasIA1++;
                else if (nombreIA === "ia2") parejasIA2++;
                else if (nombreIA === "ia3") parejasIA3++;
                else if (nombreIA === "ia4") parejasIA4++;

                if (parejasUsuario + parejasIA1 + parejasIA2 + parejasIA3 + parejasIA4 === totalParejas) {
                    finalizarPartida();
                } else {
                    setTimeout(ejecutarTurnoIA, 1000);
                }
                ...
                avanzarSiguienteTurno(nombreIA);
            }
        }, 1000);
    }, 800);
}

function avanzarSiguienteTurno(iaActual) {
    if (iaActual === "ia1") turnoActual = "ia2";
    else if (iaActual === "ia2") turnoActual = "ia3";
    else if (iaActual === "ia3") turnoActual = "ia4";
    else if (iaActual === "ia4") turnoActual = "usuario";
...
function verificarFinJuegoGeneral() {
    const totalEncontradas = parejasUsuario + parejasIA1 + parejasIA2 + parejasIA3 + parejasIA4;
    if (totalEncontradas >= totalParejas) {
        finalizarPartida();
    } else {
...
async function finalizarPartida() {
    ...
    const totalNovaPointsGanados = parejasUsuario * 10;
    ...
    let textoResultado = currentLanguage === "en" 
        ? `Game Over! You: ${parejasUsuario} | IA1: ${parejasIA1} | IA2: ${parejasIA2} | IA3: ${parejasIA3} | IA4: ${parejasIA4}`
        : `¡Partida terminada! Usuario: ${parejasUsuario} | IA 1: ${parejasIA1} | IA 2: ${parejasIA2} | IA 3: ${parejasIA3} | IA 4: ${parejasIA4}`;
