...
const traduccionesLocales = {
    "titulo_ia_facil_clasico": {
        "es": "IA Fácil (Clásico)",
        "en": "Easy AI (Classic)",
        "fr": "IA Facile (Classique)",
        "ro": "IA Ușor (Clasic)"
    },
    "btn_cerrar_sesion": {
        "es": "Cerrar sesión",
...
function aplicarTraduccionesEstaticas() {
    ...
    if (lblTitulo) {
        lblTitulo.textContent = obtenerTextoTraduccion("titulo_ia_facil_clasico", "IA Fácil (Clásico)");
    }
...
function turnoIA() {
    ...
    setTimeout(() => {
        ...
        // IA en modo fácil: elige una celda vacía al azar
        const indicesVacios = [];
        ...
        if (indicesVacios.length > 0) {
            const randomIndex = indicesVacios[Math.floor(Math.random() * indicesVacios.length)];
...
async function otorgarRecompensaVictoria() {
    ...
    try {
        ...
        // 2. Gestionar subcolección de transacciones incrementales que empiezan por "tresenraya"
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
        ...
        console.error("Error cargando idiomas.json en IaFacilClasico:", error);
    }
    ...
    try {
        ...
        console.error("Error al obtener los datos del usuario en IaFacilClasico:", error);
    }
