...
const traduccionesLocales = {
    "titulo_ia_medio_clasico": {
        "es": "IA Medio (Clásico)",
        "en": "Medium AI (Classic)",
        "fr": "IA Moyen (Classique)",
        "ro": "IA Mediu (Clasic)"
    },
    "btn_cerrar_sesion": {
        "es": "Cerrar sesión",
...
function aplicarTraduccionesEstaticas() {
    ...
    if (lblTitulo) {
        lblTitulo.textContent = obtenerTextoTraduccion("titulo_ia_medio_clasico", "IA Medio (Clásico)");
    }
...
// IA en modo medio: intenta ganar si puede, o bloquea al usuario si está a punto de ganar, de lo contrario juega aleatorio
function calcularMovimientoIA(tableroActual) {
    // 1. Ver si la IA puede ganar en este turno
    for (let combo of combinacionesGanadoras) {
        const [a, b, c] = combo;
        const valores = [tableroActual[a], tableroActual[b], tableroActual[c]];
        if (valores.filter(v => v === "O").length === 2 && valores.includes("")) {
            if (tableroActual[a] === "") return a;
            if (tableroActual[b] === "") return b;
            if (tableroActual[c] === "") return c;
        }
    }

    // 2. Ver si el usuario va a ganar en el siguiente turno y bloquearlo
    for (let combo of combinacionesGanadoras) {
        const [a, b, c] = combo;
        const valores = [tableroActual[a], tableroActual[b], tableroActual[c]];
    ...
    // 3. Tomar el centro si está libre (estrategia media básica)
    if (tableroActual[4] === "") return 4;

    // 4. Si no, elegir una celda vacía al azar
    const indicesVacios = [];
    ...
    if (indicesVacios.length > 0) {
        return indicesVacios[Math.floor(Math.random() * indicesVacios.length)];
    }
    return -1;
}
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
        ...
        console.error("Error cargando idiomas.json en IaMedioClasico:", error);
    }
    ...
    try {
        ...
        console.error("Error al obtener los datos del usuario en IaMedioClasico:", error);
    }
