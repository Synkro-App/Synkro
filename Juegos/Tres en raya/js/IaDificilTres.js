...
const traduccionesLocales = {
    "titulo_ia_dificil_tres": {
        "es": "IA Difícil (Tres en Movimiento)",
        "en": "Hard AI (Moving Tic-Tac-Toe)",
        "fr": "IA Difficile (Mouvement)",
        "ro": "IA Greu (Mișcare)"
    },
    "btn_cerrar_sesion": {
        "es": "Cerrar sesión",
...
function aplicarTraduccionesEstaticas() {
    ...
    if (lblTitulo) {
        lblTitulo.textContent = obtenerTextoTraduccion("titulo_ia_dificil_tres", "IA Difícil (Tres en Movimiento)");
    }
...
// --- MOTOR MINIMAX AVANZADO PARA UNA IA IMPLACABLE ---
function evaluarMinimax(tableroSim, profundidad, esMaximizando, fColIA, fColUser, alpha, beta) {
    const ganador = verificarGanador(tableroSim);
    if (ganador === "O") return 100 - profundidad;
    if (ganador === "X") return profundidad - 100;
    
    // Si ya colocaron todas sus piezas, limitamos la profundidad de movimientos para rendimiento perfecto
    if (fColIA >= 3 && fColUser >= 3 && profundidad >= 5) {
        return 0; // Empate técnico simulado por profundidad
    }

    if (esMaximizando) {
        let maxEval = -Infinity;
        if (fColIA < 3) {
            // Simular colocación de la IA
            for (let i = 0; i < 9; i++) {
                if (tableroSim[i] === "") {
                    tableroSim[i] = "O";
                    const evaluacion = evaluarMinimax(tableroSim, profundidad + 1, false, fColIA + 1, fColUser, alpha, beta);
                    tableroSim[i] = "";
            ...
            const indicesIA = [];
            tableroSim.forEach((val, idx) => { if (val === "O") indicesIA.push(idx); });
            const indicesVacios = [];
            tableroSim.forEach((val, idx) => { if (val === "") indicesVacios.push(idx); });
            ...
            for (let origen of indicesIA) {
                for (let destino of indicesVacios) {
                    ...
                    tableroSim[origen] = "";
                    tableroSim[destino] = "O";
                    const evaluacion = evaluarMinimax(tableroSim, profundidad + 1, false, fColIA, fColUser, alpha, beta);
                    tableroSim[destino] = "";
                    tableroSim[origen] = "O";
                    maxEval = Math.max(maxEval, evaluacion);
                    alpha = Math.max(alpha, evaluacion);
            ...
            if (!movimientoEncontrado) return 0;
        }
        return maxEval === -Infinity ? 0 : maxEval;
    } else {
        let minEval = Infinity;
        if (fColUser < 3) {
            // Simular colocación del usuario
            for (let i = 0; i < 9; i++) {
                if (tableroSim[i] === "") {
                    tableroSim[i] = "X";
                    const evaluacion = evaluarMinimax(tableroSim, profundidad + 1, true, fColIA, fColUser + 1, alpha, beta);
                    tableroSim[i] = "";
            ...
            const indicesUser = [];
            tableroSim.forEach((val, idx) => { if (val === "X") indicesUser.push(idx); });
            const indicesVacios = [];
            tableroSim.forEach((val, idx) => { if (val === "") indicesVacios.push(idx); });

            let movimientoEncontrado = false;
            for (let origen of indicesUser) {
                for (let destino of indicesVacios) {
                    movimientoEncontrado = true;
                    tableroSim[origen] = "";
                    tableroSim[destino] = "X";
                    const evaluacion = evaluarMinimax(tableroSim, profundidad + 1, true, fColIA, fColUser, alpha, beta);
                    tableroSim[destino] = "";
                    tableroSim[origen] = "X";
                    minEval = Math.min(minEval, evaluacion);
                    beta = Math.min(beta, evaluacion);
...
function turnoIA() {
    ...
    setTimeout(() => {
        ...
        let mejorMovimiento = null; // Para colocación: {index}
        let mejorOrigen = null;     // Para movimiento: {origen, destino}
        let mejorDestino = null;

        if (fichasColocadasIA < 3) {
            // Fase de colocación óptima con Minimax
            for (let i = 0; i < 9; i++) {
                if (tablero[i] === "") {
                    tablero[i] = "O";
                    let puntuacion = evaluarMinimax(tablero, 0, false, fichasColocadasIA + 1, fichasColocadasUsuario, -Infinity, Infinity);
                    tablero[i] = "";
            ...
            if (mejorMovimiento !== null && tablero[mejorMovimiento] === "") {
                tablero[mejorMovimiento] = "O";
                fichasColocadasIA++;

                const cellElement = document.querySelector(`.cell[data-index="${mejorMovimiento}"]`);
                if (cellElement) {
            ...
            // Fase de movimiento óptima con Minimax
            const indicesIA = [];
            tablero.forEach((val, idx) => { if (val === "O") indicesIA.push(idx); });
            const indicesVacios = [];
            tablero.forEach((val, idx) => { if (val === "") indicesVacios.push(idx); });

            for (let origen of indicesIA) {
                for (let destino of indicesVacios) {
                    tablero[origen] = "";
                    tablero[destino] = "O";
                    let puntuacion = evaluarMinimax(tablero, 0, false, fichasColocadasIA, fichasColocadasUsuario, -Infinity, Infinity);
                    tablero[destino] = "";
                    tablero[origen] = "O";
            ...
            if (mejorOrigen !== null && mejorDestino !== null) {
                tablero[mejorOrigen] = "";
                tablero[mejorDestino] = "O";

                const celdaOrigen = document.querySelector(`.cell[data-index="${mejorOrigen}"]`);
                const celdaDestino = document.querySelector(`.cell[data-index="${mejorDestino}"]`);
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
        ...
        console.error("Error cargando idiomas.json en IaDificilTres:", error);
    }
    ...
    try {
        ...
        console.error("Error al obtener los datos del usuario en IaDificilTres:", error);
    }
