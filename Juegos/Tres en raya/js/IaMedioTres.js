...
const traduccionesLocales = {
    "titulo_ia_medio_tres": {
        "es": "IA Medio (Tres en Movimiento)",
        "en": "Medium AI (Moving Tic-Tac-Toe)",
        "fr": "IA Moyenne (Mouvement)",
        "ro": "IA Mediu (Mișcare)"
    },
    "btn_cerrar_sesion": {
        "es": "Cerrar sesión",
    ...
    "ganador_usuario": {
        "es": "¡Has ganado! (+10 NovaPoints)",
        "en": "You win! (+10 NovaPoints)",
        "fr": "Vous avez gagné ! (+10 NovaPoints)",
        "ro": "Ai câștigat! (+10 NovaPoints)"
    },
    "ganador_ia": {
        "es": "¡La IA ha ganado!",
        "en": "AI wins!",
...
function aplicarTraduccionesEstaticas() {
    ...
    if (lblTitulo) {
        lblTitulo.textContent = obtenerTextoTraduccion("titulo_ia_medio_tres", "IA Medio (Tres en Movimiento)");
    }
...
// Función auxiliar para buscar movimientos ganadores o bloqueos directos (Nivel Medio)
function encontrarMovimientoEstrategico(tableroSimulado, fichaJugador) {
    for (let combo of combinacionesGanadoras) {
        const [a, b, c] = combo;
        const valores = [tableroSimulado[a], tableroSimulado[b], tableroSimulado[c]];
        const vacios = valores.filter(v => v === "").length;
        const fichas = valores.filter(v => v === fichaJugador).length;

        if (fichas === 2 && vacios === 1) {
            if (tableroSimulado[a] === "") return a;
            if (tableroSimulado[b] === "") return b;
            if (tableroSimulado[c] === "") return c;
        }
    }
    return null;
}

// IA en modo medio: intenta ganar o bloquear, si no, juega aleatorio
function turnoIA() {
    ...
    setTimeout(() => {
        ...
        if (fichasColocadasIA < 3) {
            ...
            // 1. Intentar ganar si tiene 2 fichas alineadas
            movimientoElegido = encontrarMovimientoEstrategico(tablero, "O");
            // 2. Si no, bloquear al usuario si tiene 2 fichas alineadas
            if (movimientoElegido === null) {
                movimientoElegido = encontrarMovimientoEstrategico(tablero, "X");
            }
            // 3. Si no hay jugada crítica, escoger al azar
            if (movimientoElegido === null || tablero[movimientoElegido] !== "") {
                const indicesVacios = [];
                tablero.forEach((val, idx) => {
                    if (val === "") indicesVacios.push(idx);
                });
                if (indicesVacios.length > 0) {
                    movimientoElegido = indicesVacios[Math.floor(Math.random() * indicesVacios.length)];
                }
            }

            if (movimientoElegido !== null && tablero[movimientoElegido] === "") {
                tablero[movimientoElegido] = "O";
                fichasColocadasIA++;

                const cellElement = document.querySelector(`.cell[data-index="${movimientoElegido}"]`);
                if (cellElement) {
            ...
            // Fase de movimiento de la IA (prueba mover una ficha para ganar o bloquear si es posible, o al azar)
            const indicesIA = [];
            tablero.forEach((val, idx) => {
            ...
            // Simular movimientos de la IA para buscar una victoria inmediata
            for (let origen of indicesIA) {
                for (let destino of indicesVacios) {
                    tablero[origen] = "";
                    tablero[destino] = "O";
                ...
                if (origenElegido !== null) break;
            }

            // Si no hay victoria directa, buscar bloqueo si el usuario puede ganar
            if (origenElegido === null) {
                for (let origen of indicesIA) {
                    for (let destino of indicesVacios) {
                        tablero[origen] = "";
                        tablero[destino] = "X"; // Simular si el usuario pusiera ahí
                        // Comprobamos si el usuario habría ganado
            ...
            // Si tampoco hay bloqueo crítico, mover al azar
            if (origenElegido === null && indicesIA.length > 0 && indicesVacios.length > 0) {
                origenElegido = indicesIA[Math.floor(Math.random() * indicesIA.length)];
                destinoElegido = indicesVacios[Math.floor(Math.random() * indicesVacios.length)];
            }

            if (origenElegido !== null && destinoElegido !== null) {
                tablero[origenElegido] = "";
                tablero[destinoElegido] = "O";

                const celdaOrigen = document.querySelector(`.cell[data-index="${origenElegido}"]`);
                const celdaDestino = document.querySelector(`.cell[data-index="${destinoElegido}"]`);
...
async function finalizarJuego(resultado) {
    ...
    if (resultado === "X") {
        if (statusMsg) statusMsg.textContent = obtenerTextoTraduccion("ganador_usuario", "¡Has ganado! (+10 NovaPoints)");
...
async function otorgarRecompensaVictoria() {
    ...
    try {
        ...
        // 1. Obtener los NovaPoints actuales actualizados de la BD para sumar 10 con precisión
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
        ...
        console.error("Error cargando idiomas.json en IaMedioTres:", error);
    }
    ...
    try {
        ...
        console.error("Error al obtener los datos del usuario en IaMedioTres:", error);
    }
