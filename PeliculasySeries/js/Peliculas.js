...
const traduccionesLocales = {
    ...
    "text_sin_peliculas": {
        "es": "No se encontraron películas disponibles.",
        "en": "No movies available were found.",
        "fr": "Aucun film disponible n'a été trouvé.",
        "ro": "Nu au fost găsite filme disponibile."
    },
    "btn_cerrar_sesion": {
        "es": "Cerrar sesión",
        "en": "Log out",
...
function aplicarTraduccionesEstaticas() {
    ...
    // Traducción exacta con la clave btn_cerrar_sesion para el botón id="btn-logout"
    const btnLogout = document.getElementById("btn-logout");
    if (btnLogout) {
        btnLogout.textContent = obtenerTextoTraduccion("btn_cerrar_sesion", "Cerrar sesión");
    }

    const lblTitulo = document.getElementById("titulo-catalogo-peliculas");
    if (lblTitulo) {
        lblTitulo.textContent = obtenerTextoTraduccion("titulo_catalogo_peliculas", "Películas");
    }
}

// --- CONSULTA Y FILTRADO ESTRICTO DE PELÍCULAS ---
async function cargarCatalogoPeliculas() {
    const contenedorGrid = document.getElementById("contenedor-grid-peliculas");
    ...
    try {
        ...
        const qPeliculas = query(psRef, where("tipo", "==", "película"));
        const querySnapshot = await getDocs(qPeliculas);

        if (querySnapshot.empty) {
            contenedorGrid.innerHTML = `<p style="color: #000; font-style: italic;">${obtenerTextoTraduccion("text_sin_peliculas", "No se encontraron películas disponibles.")}</p>`;
            return;
        }

        let peliculasVisiblesCount = 0;

        querySnapshot.forEach((docSnap) => {
            const peliculaData = docSnap.data();

            // 1. FILTRO DE EDAD: Si Edad usuario < edadMinima, NO se muestra directamente
            const edadMinima = peliculaData.edadMinima !== undefined ? Number(peliculaData.edadMinima) : 0;
            if (edadUsuarioGlobal < edadMinima) {
                return; // Omitir / No mostrar
            }

            // 2. FILTRO DE TIPO Y SUSCRIPCIÓN (Gratis vs Pago)
            const tipoAcceso = peliculaData.type || "Gratis";
            if (tipoAcceso === "Pago") {
                const arraySubsObra = Array.isArray(peliculaData.subs) ? peliculaData.subs : [];
            ...
            const tarjetaPeli = crearTarjetaPelicula(peliculaData);
            contenedorGrid.appendChild(tarjetaPeli);
            peliculasVisiblesCount++;
        });

        if (peliculasVisiblesCount === 0) {
            contenedorGrid.innerHTML = `<p style="color: #000; font-style: italic;">${obtenerTextoTraduccion("text_sin_peliculas", "No se encontraron películas disponibles.")}</p>`;
        }

    } catch (error) {
        console.error("Error al consultar el catálogo de películas:", error);
    }
}

function crearTarjetaPelicula(peliculaData) {
    ...
    imgPoster.src = normalizarSourceImagen(peliculaData.imgCartelera);
    ...
    if (peliculaData.titulo && typeof peliculaData.titulo === "object") {
        textoTituloLocal = peliculaData.titulo[currentLanguage] || peliculaData.titulo["es"] || Object.values(peliculaData.titulo)[0] || "";
    } else if (typeof peliculaData.titulo === "string") {
        textoTituloLocal = peliculaData.titulo;
    }
    ...
    // --- EVENTO DE Clic: GUARDAR EN LOCALSTORAGE Y NAVEGAR A Película.html ---
    card.addEventListener("click", () => {
        let tituloEnEspanol = "";
        if (peliculaData.titulo && typeof peliculaData.titulo === "object") {
            tituloEnEspanol = peliculaData.titulo["es"] || peliculaData.titulo[currentLanguage] || Object.values(peliculaData.titulo)[0] || "";
        } else if (typeof peliculaData.titulo === "string") {
            tituloEnEspanol = peliculaData.titulo;
        }
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
        ...
        console.error("Error cargando idiomas.json en Peliculas:", error);
    }
    ...
    onSnapshot(qUsuario, async (querySnapshot) => {
        if (!querySnapshot.empty) {
            ...
            // Una vez obtenidos los datos del usuario, cargamos y filtramos el catálogo
            await cargarCatalogoPeliculas();
        }
    }, (error) => {
        console.error("Error en tiempo real con el usuario en Peliculas:", error);
    });
