...
function aplicarTraduccionesEstaticas() {
    ...
    const lblTitulo = document.getElementById("titulo-catalogo-favoritas");
    if (lblTitulo) {
        lblTitulo.textContent = obtenerTextoTraduccion("titulo_catalogo_favoritas", "Favoritas");
    }
}
...
// --- CARGAR SUBCOLECCIÓN Películas y Series Favoritas ---
async function cargarSubcoleccionFavoritas(userId) {
    const contenedorGrid = document.getElementById("contenedor-grid-favoritas");
    if (!contenedorGrid) return;

    contenedorGrid.innerHTML = "";

    try {
        const favoritasRef = collection(db, "Usuarios", userId, "Películas y Series Favoritas");
        const querySnapshot = await getDocs(favoritasRef);

        if (querySnapshot.empty) {
            contenedorGrid.innerHTML = `<p style="color: #000; font-style: italic;">${obtenerTextoTraduccion("text_sin_favoritas", "No hay elementos en esta lista.")}</p>`;
            return;
        }
        ...
        querySnapshot.forEach((docSnap) => {
            ...
            const tarjetaItem = crearTarjetaItemFavorita(itemId, itemData);
            contenedorGrid.appendChild(tarjetaItem);
        });

    } catch (error) {
        console.error("Error al consultar la subcolección Películas y Series Favoritas:", error);
    }
}

function crearTarjetaItemFavorita(itemId, itemData) {
    const card = document.createElement("div");
    card.className = "pelicula-card";

    // Imagen / Poster del elemento (soporte para imgCartelera, img o imagen)
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
        ...
        console.error("Error cargando idiomas.json en Favoritas:", error);
    }
    ...
    try {
        ...
        if (!querySnapshot.empty) {
            ...
            await cargarSubcoleccionFavoritas(userDocIdGlobal);
        }
    } catch (error) {
        console.error("Error al obtener los datos del usuario en Favoritas:", error);
    }
